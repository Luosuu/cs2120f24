...
def andCommutative_bottom_up : (P ∧ Q) ↔ (Q ∧ P) :=
  let forward := fun h =>
    let q := h.right
    let p := h.left
    And.intro q p
...
def orCommutative_bottom_up : (P ∨ Q) ↔ (Q ∨ P) :=
  let forward := fun h =>
    let handle_p := fun p => Or.inr p
    let handle_q := fun q => Or.inl q
    Or.elim h handle_p handle_q

  let backward := fun h =>
    let handle_q := fun q => Or.inr q
    let handle_p := fun p => Or.inl p
    Or.elim h handle_q handle_p
...
def identityAnd_bottom_up : (P ∧ True) ↔ P :=
  let forward := fun h =>
    let p := h.left
    p
...
def identityOr_bottom_up : (P ∨ False) ↔ P :=
  let forward := fun h =>
    let handle_p := fun p => p
    let handle_f := fun f => False.elim f
    Or.elim h handle_p handle_f
...
def annhilateAnd_bottom_up : (P ∧ False) ↔ False :=
  let forward := fun h =>
    let f := h.right
    False.elim f
...
def annhilateOr_bottom_up : (P ∨ True) ↔ True :=
  let forward : (P ∨ True) → True :=
    fun h =>
      let _ := h        -- explicitly show we ignore the input
      let t := True.intro
      t

  let backward : True → (P ∨ True) :=
    fun t =>
      let right_case := t
      Or.inr right_case
...
def orAssociative_bottom_up : ((P ∨ Q) ∨ R) ↔ (P ∨ (Q ∨ R)) :=
  -- Forward direction
  let forward := fun porq_or =>
    let handle_porq := fun porq =>
      let handle_p := fun p => Or.inl p
      let handle_q := fun q => Or.inr (Or.inl q)
      Or.elim porq handle_p handle_q

    let handle_r := fun r =>
      Or.inr (Or.inr r)

    Or.elim porq_or handle_porq handle_r

  -- Backward direction
  let backward := fun por_qorr =>
    let handle_p := fun p =>
      Or.inl (Or.inl p)

    let handle_qorr := fun qorr =>
      let handle_q := fun q =>
        Or.inl (Or.inr q)
      let handle_r := fun r =>
        Or.inr r
      Or.elim qorr handle_q handle_r

    Or.elim por_qorr handle_p handle_qorr
...
def andAssociative_top_down  : ((P ∧ Q) ∧ R) ↔ (P ∧ (Q ∧ R)) :=
Iff.intro
  -- forward: from (P ∧ Q) ∧ R show P ∧ (Q ∧ R), in bottom up proof style
  (fun (pandq_andr : (P ∧ Q) ∧ R) =>
    And.intro
      pandq_andr.left.left
      (And.intro
        pandq_andr.left.right
        pandq_andr.right)
  )
...
def exportation_top_down : ((P ∧ Q) → R) ↔ (P → Q → R) :=
Iff.intro
  ...
  (fun h =>
    fun pandq =>
      h (And.left pandq) (And.right pandq))
...
def absurdity_bottom_up : (P → Q) ∧ (P → ¬Q) → ¬P :=
  fun h => fun p =>
    let q : Q := h.left p
    let nq : ¬Q := h.right p
    nq q
...
def distribNotAnd_top_down : ¬(P ∧ Q) ↔ (¬P ∨ ¬Q) :=
Iff.intro
  ...
  (fun h =>
    ...
    | Or.inl p =>
        ...
        | Or.inl q => nomatch (h (And.intro p q))
        | Or.inr nq => Or.inr nq
    | Or.inr np => Or.inl np)
  ...
  (fun h pandq =>
    match h with
    | Or.inl np => np pandq.left
    | Or.inr nq => nq pandq.right)
...
def distribNotOr_top_down : ¬(P ∨ Q) ↔ (¬P ∧ ¬Q) :=
  Iff.intro
    -- forward: from ¬(P ∨ Q) show (¬P ∧ ¬Q)
    (fun n_porq =>
      And.intro
        (fun p => n_porq (Or.inl p))
        (fun q => n_porq (Or.inr q)))

    -- backward: from (¬P ∧ ¬Q) show ¬(P ∨ Q)
    (fun npandnq => fun porq =>
      Or.elim porq
        (fun p => npandnq.left p)
        (fun q => npandnq.right q))
