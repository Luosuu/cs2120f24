...
def evalArithBinOp: ArithBinOp → (Nat → Nat → Nat)
...
def evalArithUnop: ArithUnOp → (Nat → Nat)
| (ArithUnOp.fac) => fac
...
| ArithExpr.unOp op e,                i => evalArithUnop op (arithEval e i)
| ArithExpr.binOp op e1 e2,           i => evalArithBinOp op (arithEval e1 i) (arithEval e2 i)
